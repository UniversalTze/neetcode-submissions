    ...
    def exist(self, board: List[List[str]], word: str) -> bool:
        def dfs(count, position, xbound, ybound):
            ...
            if board[position[0]][position[1]] == word[count]:
                ...
                exist = dfs(count, [position[0] + 1, position[1]], xbound, ybound) or \
                ...
                if exist:
                    return exist
                prevpositions.remove(pos) # remove seen positions when false, so that we can check
                # other potential paths in DFS.
        found = False
