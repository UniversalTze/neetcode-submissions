    ...
    def longestPalindrome(self, s: str) -> str:
        ...
        for index in range(len(s)):
            ...
            while left >= 0 and right < len(s) and s[left] == s[right]:
                ...
                # remember to plus 1 as right and left are indexes
                # e.g if right = 7 and left = 9
                # differences in index is 2, but actual length is 3
                if length < right - left + 1: 
                    # therefore we're dealing with proper lengths and no need 
                    # increment 1 to upper domain when string slicing
                    length = right - left + 1
                    longindex = left
                left -= 1
                right += 1
            ...
            while left >= 0 and right < len(s) and s[left] == s[right]:
                ...
                left -= 1
                right += 1
        # also note when string slicing, the upper index/domain is noninclusive (exclusive)
        return s[longindex:longindex + length]
